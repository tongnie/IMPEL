...
num_unknown_nodes = 5  # 5 for JL and 10 for others
_, _, n_node, _ = train_ha.shape
n_u = num_unknown_nodes
rand = np.random.RandomState(42)  # Fixed random output, just an example when seed = 0.
unknown_set = rand.choice(list(range(0, n_node)), n_u, replace=False)
unknown_set = set(unknown_set)

missing_index = np.ones(train_ha.shape)
missing_index[:, :, list(unknown_set), :] = 0
train_ha = train_ha * missing_index
###############################
