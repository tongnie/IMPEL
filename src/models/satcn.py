...
from src.layers.st_aggregators_new import AGGREGATORS
from src.layers.st_aggregators_mask import AGGREGATORS_MASK
from src.utils.scaler import SCALERS
...
class tcn_layer(nn.Module):
    def __init__(self, kt, c_in, c_out, act="linear", dropout = 0.1):
        ...
        if self.act == "GLU":
            self.conv = nn.Conv2d(c_in, c_out * 2, (kt, 1), padding=(1, 0))
        else:
            self.conv = nn.Conv2d(c_in, c_out, (kt, 1), padding=(1, 0))
...
class SATCN(BaseModel):
    def __init__(self,
                 ...
                 dropout=0,
                 **args):
        super(SATCN, self).__init__(**args)
        self.s_layer0 = STower(in_variables, channels, aggragators + ['distance','d_std'], ['identity'],avg_d, self.device, masking, dropout)
        ...
        for i in range(layers):
            self.s_convs.append(STower(channels, channels, aggragators, scalers, avg_d, self.device, False, dropout))
            self.t_convs.append(tcn_layer(t_kernel, channels, channels, dropout))
        self.out_conv = nn.Conv2d(channels, in_variables, (1, 1), 1)


    def forward(self, x, supports):
        # :param X: Input data of shape [b s n c] -> (b, c, s, n)
