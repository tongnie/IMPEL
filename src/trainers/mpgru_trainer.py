...
class MPGRU_Trainer(BaseTrainer):
    def __init__(self,
                 unknown_set,
                 known_set,
                 n_m,
                 llm_encoding,  ##
                 **args):
        super(MPGRU_Trainer, self).__init__(**args)
    ...
    def train_batch(self, X, label, iter):
        ...
        llm_encoding = llm_encoding[list(self._known_set), :]
        ...
        pred = self.model(X, supports, llm_encoding)
    ...
    def test_batch(self, X, label):
        pred = self.model(X, self.supports, self._llm_encoding)  ##
        pred, label = self._inverse_transform([pred, label])
        return pred, label
